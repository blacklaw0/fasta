...
/*  $Id: nmgetlib.c 965 2012-06-27 10:05:41Z wrp $ */
/* $Revision: 965 $  */
...
open_lib(struct lib_struct *lib_p, int ldnaseq, int *sascii, int outtty)
{
 ...
 l1:
  ...
  if (!use_stdin && (lib_type <= LASTTXT) && (getlibam[lib_type]!=NULL)) {
	...
	m_fptr->lb_type = lib_type;
	m_fptr->getlib = getlibam[lib_type];
	m_fptr->ranlib = ranlibam[lib_type];
	m_fptr->mm_flg = 1;
	return m_fptr;
      }
...
*/
  ...
  /*	preserve ->lline for easier re-use */
  /*
  ...
  re_openlib(struct lmf_str *om_fptr, struct lib_struct *lib_p, int outtty)
{
  ...
  if (om_fptr == last_m_fptr) {return om_fptr;}
  else { closelib(last_m_fptr,1); }

  last_m_fptr = om_fptr;
  /* if the file mmap()ed and has been opened - use it and return */
  if (om_fptr->mm_flg && om_fptr->libf) {
    return om_fptr;
  }
#ifdef MYSQL_DB
