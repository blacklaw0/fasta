...
/*  $Id: mshowalign2.c 979 2012-07-20 19:36:28Z wrp $ */
/* $Revision: 979 $  */
		...
		)
{
  ...
  char tmp_str[20];
  char info_str[200];
  char bline[2048], *bline_p, *bl_ptr, *bp, fmt[40];
  ...
  if ( /* !(m_msp->stages > 1 || m_msp->markx & MX_M9SUMM) */ !m_msp->align_done) {
    if (!m_msp->pre_load_done) { pre_load_best(aa1save, maxn, bptr, istop, m_msp); }
  ...
  for (ib=0; ib<istop; ib++) {
	...
	exit(1);
      ...
      ranlib_done = 0;
      strncpy(bline, bbp->mseq->bline, sizeof(bline));
    }
	...
	ranlib_done = 1;
      ...
      n1 = re_getlib(aa1save, m_msp->ann_flg ? &(bbp->seq->aa1_ann) : NULL, maxn,
    ...
    */
    ...
    strncpy(name1,bline_p,sizeof(name1));
  ...
  /* l_name is used to build an HTML link from the bestscore line to
    ...
    if (m_msp->markx & MX_MBLAST) { strncpy(name1,"Sbjct",sizeof(name1)-1);}
    ...
    if (m_msp->markx & MX_MBLAST) { strncpy(name0,"Query",sizeof(name0)-1);}
    name0[nml]='\0';
    ...
    if (m_msp->markx & MX_M11OUT) {
      qt_offset = m_msp->q_offset + (m_msp->q_off-1)+(m_msp->sq0off-1);
      lt_offset = loffset + (bbp->seq->l_off-1) + (m_msp->sq1off-1);
      fprintf (fp, "s {\n   \"%s\" %ld %ld \n   \"%s\" %ld %ld\n}\n",
	       name0, qt_offset, qt_offset + m_msp->n0-1,
	       name1, lt_offset, lt_offset + bbp->seq->n1 - 1);
      fprintf (fp, "h {\n   \"%s\"\n   \"%s\"\n}\n", m_msp->qtitle, bline_p);
    }
    else if ((m_msp->markx & MX_ATYPE)!=7 && !(m_msp->markx & MX_M10FORM)) {
      fprintf (fp, fmt,bline_p,n1tot);	 /* provides >>id  description (length) line */
    }
	...
	lc=calc_code(aa0[bbp->frame],m_msp->n0,
		     ...
		     seqc0a, maxc,
		     f_str[bbp->frame],m_msp->show_code);
	...
	fprintf(fp, "\n Score = %.1f bits (%d),  Expect = %.1g\n",
		lbits, rst_p->score[ppst->score_ix],
	...
	if (m_msp->nframe > 2) 
	  ...
	  fputs("rev-comp",fp);
	  name0[nml-1]='\0';
	  if (!(m_msp->markx & MX_MBLAST)) strncat(name0,"-",sizeof(name0)-1);
	}

	if (m_msp->arelv > 0)
	  fprintf (fp, " %s: %3d", m_msp->alab[0],rst_p->score[0]);
	if (m_msp->arelv > 1)
	  fprintf (fp, " %s: %3d", m_msp->alab[1],rst_p->score[1]);
	if (m_msp->arelv > 2)
	  fprintf (fp, " %s: %3d", m_msp->alab[2],rst_p->score[2]);
	...
	fprintf (fp, "; %s_%s: %3d\n", m_msp->f_id0,m_msp->alab[0],cur_ares_p->rst.score[0]);
	if (m_msp->arelv > 1)
	  fprintf (fp,"; %s_%s: %3d\n", m_msp->f_id0,m_msp->alab[1],cur_ares_p->rst.score[1]);
	if (m_msp->arelv > 2)
	  fprintf (fp,"; %s_%s: %3d\n", m_msp->f_id0,m_msp->alab[2],cur_ares_p->rst.score[2]);
