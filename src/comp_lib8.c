...
/*  $Id: comp_lib8.c 978 2012-07-20 17:48:41Z wrp $ */
/*  $Revision: 978 $  */
...
struct seqr_chain *
new_seqr_chain(int max_chain_seqs, int aa1b_size, struct seqr_chain *old_chain, int maxn, long *lost_memK);
...
main (int argc, char *argv[]) 
{
  ...
  /* initialize values in comp_lib8 that are thread/serial specific */
  fset_vars(&m_msg, &pst);
  ...
  */
  ...
  /* in comp_lib8.c, getlib_info saves all the state information
  ...
  while(1) {
	...
	*aa0[1]='\0';
	...
	if (bestp_arr[i]->n1 != bestp_arr[i]->seq->n1) {
	  fprintf(stderr," *** [comp_lib8.c:1105] *** n1 conflict[%d]: n1: %d != seq->n1: %d\n",
  ...
  l4:   
	...
	if (m_msg.mshow >= 0) { m_msg.nshow = m_msg.mshow; }
    ...
    /* **************************************************************** */
    /* check for additional sequences via link script                   */
    /* **************************************************************** */
	...
	for (i=0; i < m_msg.nshow; i++) {
	  if (bestp_arr[i]->seq->aa1b != NULL) {
	    ...
	    if (bestp_arr[i]->n1 != bestp_arr[i]->seq->n1) {
	      fprintf(stderr,"[*** comp_lib8.c:1319***] -n1:%d != seq->n1:%d\n",
    ...
    num_reader_bufs = m_bufi.max_work_buf;
      ...
      for (i=0; i<m_msg.nshow; i++) {
	if (bestp_arr[i]->n1 != bestp_arr[i]->seq->n1) {
	  fprintf(stderr," *** [comp_lib8.c:1355] *** n1 conflict[%d]: n1: %d != seq->n1: %d\n",
		  i, bestp_arr[i]->n1, bestp_arr[i]->seq->n1);
	}
      }
#endif
      ...
      for (i=0; i<m_msg.nshow; i++) {
	if (bestp_arr[i]->n1 != bestp_arr[i]->seq->n1) {
	  fprintf(stderr," *** [comp_lib8.c:1367] *** n1 conflict[%d]: n1: %d != seq->n1: %d\n",
		  i, bestp_arr[i]->n1, bestp_arr[i]->seq->n1);
	}
      }
#endif
...
#ifdef DEBUG
  printf("[*** comp_lib8.c***] seqr_chains: %d [%d]; aa1b_buffer: %ld [%ld] blocks (lost: %ld); using memory:%d\n",
...
void fsigint()
{
  ...
  print_sum(stdout,&qtt, &db,0,0L);
  if (outfd!=stdout) print_sum(outfd,&qtt, &db,0,0L);
...
/* new_seqr_chain() allocates space for:
     ...
     unsigned char aa1b_base[aa1b_size]

   comp_lib8.c tracks how much space is being used, and, if the space
   is too large, reuses existing buffers.
*/
/* **************************************************************** */
struct seqr_chain *
new_seqr_chain(int max_chain_seqs, int aa1b_size, struct seqr_chain *old_seqr_chain,
	       int maxn, long *lost_memK) {
...
free_seqr_chain(struct seqr_chain *this_seqr_chain) {
  ...
  for (cur_seqr_chain_p = this_seqr_chain; cur_seqr_chain_p != NULL;
    ...
    free(--cur_seqr_chain_p->aa1b_base);
		...
		struct mngmsg *m_msp, const struct pstruct *ppst)
{
  ...
  if (getlib_info->eof) {
    /* if we are short of memory, close the file we are done with */
    if (getlib_info->use_memory < 0) {
  ...
  /* this section transitions between the comp_lib6,7 strategy, which
     ...
     last seqr_chain that it used.  Linking seqr_chains and thread
  ...
  if ((getlib_info->use_memory >= 0) && 
      (getlib_info->start_seqr_chain == NULL || m_msp->cur_seqr_cnt < m_bufi_p->max_work_buf ||
       getlib_info->tot_memK + ((m_bufi_p->seq_buf_size+1)>>10) < getlib_info->max_memK)
      ) {
    my_seqr_chain = 
      new_seqr_chain(m_bufi_p->max_chain_seqs,(m_bufi_p->seq_buf_size+1),
		     getlib_info->cur_seqr_chain, m_msp->ldb_info.maxn, &getlib_info->lost_memK);
...
/* **************************************************************** */
/* seqr_chain_work() (comp_lib8.c, comp_lib7e.c) corresponds to     */
...
seqr_chain_work(unsigned char **aa0, unsigned char *aa0s, struct buf_head *lib_bhead_p,
  ...
  struct seqr_chain *lib_seqr_chain, *current_seqr_chain_p;
  ...
  while ((current_seqr_chain_p 
    ...
    /* to enable the transition from everything in memory (comp_lib7e)
       to buffers read as neeed (comp_lib5e), comp_lib8.c requires that
