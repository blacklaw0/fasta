...
/*  $Id: initfa.c 1044 2012-08-19 19:28:02Z wrp $ */
/*  $Rev: 1044 $  */
...
#ifdef PCOMPLIB
char *verstr="36.3.5d Aug, 2012 MPI";
#else
char *verstr="36.3.5d Aug, 2012";
#endif
...
init_pam2 (struct pstruct *ppst) {
  ...
  /* add values for 'J' (I/L) value, which are not present in 1-D matrices */
    ...
    if (ix_j <= ppst->nsq) {
      ...
      /* do not assume symmetric matrices */
      for (i=1; i <= nsq; i++) {
	ppst->pam2[0][ix_j][i] = ppst->pam2[0][ix_i][i];
	ppst->pam2[0][i][ix_j] = ppst->pam2[0][i][ix_i];
      }
    }
    else {
      pascii['O'] = pascii['K'];
	  ...
	  struct mngmsg *m_msg, struct pstruct *ppst)
{
  ...
  case 's':
    ...
    /* initpam() can reset the sequence type */
	...
	if (ppst->dnaseq >= SEQT_DNA && m_msg->qdnaseq == SEQT_UNK) {
	  m_msg->qdnaseq = ppst->dnaseq;
	}
      }
    }
    ppst->pam_set=1;
...
DNA sequence library.  Do not use a DNA query/scoring matrix.\n",prog_func);
  ...
  */
  ...
  /* no init_ascii() because we translate lower case library sequences */
    ...
    if (m_msg->ann_flg && strchr(m_msg->ann_arr,'*')) {lascii['*'] = NA;}
  ...
  if (m_msg->ann_flg) {
    ...
    if (strchr(m_msg->ann_arr,'*')) {l_ann_ascii['*'] = NA;}
...
last_params(unsigned char *aa0, int n0, 
  ...
  if (ppst->ext_sq_set) { ppst->nsq_e = nsq = ppst->nsqx; }
...
validate_params(const unsigned char *aa0, int n0, 
  ...
  for (i=0; i<= ppst->nsq; i++) {
    if (ppst->pam2[0][0][i] > -1000) {
      fprintf(stderr," *** ERROR ***  pam2[0][0][%d] == %d\n",
	      i,ppst->pam2[0][0][i]);
      good_params = 0;
    }
    if (ppst->pam2[0][i][0] > -1000) {
      fprintf(stderr," *** ERROR ***  pam2[0][%d][0] == %d\n",
	      i,ppst->pam2[0][i][0]);
      good_params = 0;
    }
  }
